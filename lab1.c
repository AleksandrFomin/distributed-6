...
int send(void * self, local_id dst, const Message * msg){
	...
	switch(msg->s_header.s_type){
		case TRANSFER:
			sprintf(msg_text, log_transfer_out_fmt, get_lamport_time(),
			...
			increment_time();
			break;
		case ACK: case STOP: case STARTED:
			increment_time();
			break;
		default:
			break;
...
int send_message(int*** matrix, local_id proc_id, int N, 
	...
	switch(type){
		case STARTED:
			sprintf(msg_text, log_started_fmt, get_lamport_time(),
				proc_id, getpid(), getppid(), balance);
			break;
		case DONE:
			sprintf(msg_text, log_done_fmt, get_lamport_time(),
				proc_id, balance);
			break;
		default:
			break;
	...
	msg = create_message(type, msg_text, strlen(msg_text));
...
int receive(void * self, local_id from, Message * msg){
	...
	if(read(pipe_fd, &(msg->s_header), sizeof(MessageHeader)) ==-1){
		return -1;
	}
	if(read(pipe_fd, &(msg->s_payload), msg->s_header.s_payload_len) ==-1){
		return -1;
	}
	switch(msg->s_header.s_type){
		case TRANSFER:
			set_new_time(msg->s_header.s_local_time);
			increment_time();
			sprintf(msg_text, log_transfer_in_fmt, get_lamport_time(),
